...
1. In Board, read `TaskContext` with `useContext`, guard against a missing provider, and compute the number of DONE tasks from the shared task array.
2. Use `useEffect` to set `document.title` to **Sprint board — N done**.
3. Include the completed count in the effect’s dependency array.
