...
window.__mount = (Component) => {
  ...
  function Host() {
    const [room, setRoom] = React.useState("Break room"),
      [orders, setOrders] = React.useState<string[]>([]);
    ...
    return (
      <>
        <Preview
          name="Human"
          room={room}
          onOrder={(text: string) => setOrders((a) => [...a, text])}
        />
        {window.__VARIANT === "6-1" && (
          <button
            onClick={() =>
              setRoom((r) =>
                r === "Break room" ? "Server room" : "Break room",
              )
            }
          >
            Change room
        ...
        {window.__VARIANT === "9-0" && (
          <p>Parent received: {orders.join(", ") || "No orders yet"}</p>
        )}
      </>
    );
  }
  ...
  setTimeout(() => {
    ...
    const count = (tag: string) => root.querySelectorAll(tag).length;
    const expected: Record<string, boolean> = {
      "0-0": count("h1") === 1 && !!root.querySelector("h1[class]"),
      "0-1": count("h1") === 1 && count("span") === 1,
      "1-0": count("li") === 1,
      "1-1": count("li") >= 1,
      "2-0": count("span") === 2,
      "2-1": count("span") === 1,
      "3-0": count("h2") === 1,
      "3-1": root.textContent?.includes("HUMAN") || false,
      "4-0": count("button") === 1,
      "4-1": count("input") === 1,
      "5-0": count("input") === 1 && count("button") === 1,
      "5-1": count("button") === 1,
      "7-0": count("input") === 1 && count("button") === 1,
      "8-0": count("button") === 1,
      "8-1": count("button") === 2,
      "9-1": count("p") === 2,
      "10-0": root.textContent?.includes("amber") || false,
      "2-2": count("h1") === 1 && count("section") === 1,
      "1-2": count("li") === 1,
      "8-2": count("input") === 1 && count("button") === 1,
    };
    notify(
      "rendered",
      JSON.stringify({
        text: root.innerText,
        valid: root.children.length > 0 && (expected[window.__VARIANT] ?? true),
      }),
    );
  }, 150);
};
