...
Each page also requires two different, topic-specific B.U.G. lines in its catalog entry: `jokes.opened` and `jokes.read`. Keep them short and in his dry office-supervisor voice. He tells the first when the page opens and the second when the reader reaches the bottom, once per visit. The opening line stays visible for at least four seconds before the completion line can replace it, including when all text fits without scrolling. Leaving the page cancels its pending line; inactive Help pauses delivery. These remarks temporarily cover the story and return to it when dismissed.
